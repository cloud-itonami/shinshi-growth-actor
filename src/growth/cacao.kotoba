(ns growth.cacao
  "Agent-side CACAO issuance (JVM). The shinshi-growth-actor mints its OWN
  ...
  `shinshi.cacao` (`jk-luxury/club-shinshi/20-actors/shinshi`, keep in sync);
  the SIWE/wire builders below are a faithful copy of the proven byte-exact
  pure functions in `kotoba.cacao` (kotoba-auth / kotoba-wasm), and the
  crypto is JDK Ed25519 + a minimal CBOR encoder.

  Per-actor key model: this actor generates + persists its OWN Ed25519 key
  ...
  The private key is persisted to `.growth/identity.edn` (gitignored) by
  convention — NEVER commit a private key. `load-or-create-identity!` takes
  the path as an argument; it is the CALLER's decision where to persist,
  not a namespace constant here.

  JVM-only I/O namespace by convention (.clj): there is no kototama/cljs
...
;; to `kotobase.cid/canonical-graph`. Ported from shinshi.cacao.
...
(defn canonical-graph
  "The deterministic graph CID for one of this actor's databases. The edge
...
(def default-db-name
  "This actor's own ledger domain (see `growth.store`'s append-only audit
  ledger) — the growth-loop-as-a-service database."
  "growth")
...
(defn load-or-create-identity!
  "Per-actor key: load this actor's persisted Ed25519 identity at `path`, or
